...
if not DB_USER or not DB_PASSWORD:
    st.error("CREDENCIALES FALTANTES: Define DB_USER y DB_PASSWORD en el entorno")
    st.stop()
...
engine_users = engine_products = engine_inventory = engine_cart = engine_orders = None
...
try:
    ...
    engine_cart = create_db_engine('cart_db')
    engine_orders = create_db_engine('order_db')
except Exception as e:
...
def get_data():
    ...
    try:
        if any(v is None for v in [engine_users, engine_products, engine_inventory, engine_cart, engine_orders]):
            logger.error("Conexiones no disponibles")
            return {k: None for k in ['users', 'products', 'inventory', 'cart', 'orders']}
        ...
        cart = pd.read_sql("""
            SELECT c.usuario_id, ci.producto_id, ci.cantidad
            FROM carts_tb c, jsonb_to_recordset(c.items) AS ci(producto_id bigint, cantidad int)
        """, engine_cart) if engine_cart else None
        
        orders = pd.read_sql("""
            SELECT id, usuario_id, estado, total, fecha_creacion
            FROM orders_tb
        """, engine_orders) if engine_orders else None
        
        logger.info(f"✓ Datos cargados: {len(users)} usuarios, {len(products)} productos, {len(inventory)} inventarios, {len(orders) if orders is not None else 0} órdenes")
        return {'users': users, 'products': products, 'inventory': inventory, 'cart': cart, 'orders': orders}
...
try:
    ...
    with col3:
        if df_inv is not None and not df_inv.empty:
            ...
            st.metric("Alertas de Stock", bajo_stock, delta_color="inverse")
        else:
    ...
    with col5:
        total_orders = len(df_orders) if df_orders is not None else 0
        st.metric("Total Órdenes", total_orders)
    ...
    st.divider()
    st.subheader("Órdenes y Carritos")
    
    oc1, oc2 = st.columns(2)
    with oc1:
        st.subheader("Órdenes por Estado")
        if df_orders is not None and not df_orders.empty and 'estado' in df_orders.columns:
            fig_orders = px.pie(df_orders, names='estado', hole=0.4,
    ...
    with oc2:
        st.subheader("Resumen de Carritos")
        if df_cart is not None and not df_cart.empty:
            total_cart_items = len(df_cart)
            unique_cart_users = df_cart['usuario_id'].nunique()
            st.metric("Items en Carritos", total_cart_items)
            st.metric("Usuarios con Carrito", unique_cart_users)
        else:
            st.info("No hay datos de carritos")
